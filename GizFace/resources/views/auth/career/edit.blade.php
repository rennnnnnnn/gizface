...
<div class="container">
    ...
    <div class="row" style="margin-left: 160px;margin-top:40px;">
        <form name="f1" id="form1" action="" target="_self" class="form-horizontal career-form" method="get">
            ...
            <div class="form-group">
                ...
                @endif
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="company" name="company" value="{{old('company',$careerInfo->company)}}" placeholder="会社名を入力して下さい。">
                </div>
            </div>
            <div class="form-group">
                ...
                @endif
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="jobTitle" name="jobTitle" value="{{old('jobTitle',$careerInfo->job_title)}}" placeholder="業務概要を入力して下さい。">
                </div>
            </div>
            <div class="form-group">
                    ...
                    <div class="row">
                            ...
                            <div class="form-group" id="datepicker-daterange">
                                ...
                                <div class="col-sm-7 form-inline" style="padding-left:0;">
                                    <div class="input-group date">
                                        @if(!empty($careerInfo->from))
                                        <?php $from = $careerInfo->from->format('Y/m/01') ?>
                                        @else
                                        <?php $from = null ?>
                                        @endif
                                      <input type="text" class="form-control datepicker" name="from" id="from" value="{{old('from',$from)}}">
                        ...
                        <div class="col-sm-5">
                            <div class="form-group" id="datepicker-daterange">
                                ...
                                <div class="col-sm-7 form-inline" style="padding-left:0;">
                                    <div class="input-group date">
                                        @if(!empty($careerInfo->to))
                                        <?php $to = $careerInfo->to->format('Y/m/01') ?>
                                        @else
                                        <?php $to = null ?>
                                        @endif
                                      <input type="text" class="form-control datepicker" name="to" id="to" value="{{old('to',$to)}}">
                                      <div class="input-group-addon">
                                        <i class="fa fa-calendar"></i>
                                      </div>
                                    </div>
            ...
            <div class="form-group">
                ...
                @endif
                <div class="col-sm-6">
                <textarea class="row-5 form-control" id="jobDetail" name="jobDetail" placeholder="箇条書きで入力して下さい。">{{old('jobDetail',$careerInfo->job_detail)}}</textarea>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-sm-1">
                    <div class="flex">
                    <input type="number" class="form-control" name="scale" id="scale" value="{{old('scale',$careerInfo->team_scale)}}">
    ...
    <div class="text-center">
        <button type="button" class="btn" id="delete">削除</button>
...
$(function(){
    ...
    function onLoad(){
        ...
        var oldTool = @json(old('tool'));
         ...
         // 取得したスキルリスト
        var careerSkill = JSON.parse(@json($careerSkill)); // phpから受け取ったスキルデータ
        var processInfo = JSON.parse(@json($processInfo)); // phpから受け取った工程データ
        ...
        // 初期遷移時
            ...
            $.each(careerSkill,function(index,val){
               if(index === 'language'){
                   $.each(val,function(key,skill){
                        var prefix = 'language'
                        var inputCount = languageCount;
                        setInputList(prefix,inputCount,skill);
                        languageCount++;
                   });
               }
            });
        ...
        if(oldProcess !== null){
            ...
            $.each(processInfo,function(i,e){
                var num = parseInt(i)+1;
                if(num > 1){
                    $('.processAddForm').trigger('click');
                }
                $('[name=process\\[' + num + '\\]] option[value="' + e.process + '"]').prop('selected','selected')
            })
        }
    ...
    $('#update').on('click',function(){
        $('#form1').attr("action","{{ route('career.update') }}");
    ...
    $('#delete').on('click',function(){
        if(confirm("削除します。\nよろしいですか。")){
        $('#form1').attr("action","{{ route('career.delete') }}");
        $('#form1').attr("target","_self");
        $('#form1').attr("method","post");
        $('#form1').submit();
        }else{
